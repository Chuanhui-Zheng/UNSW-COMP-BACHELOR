...
void IntListInsertInOrder(IntList l, int v) {
	...
	//CREATE NODE WITH intlistnode TYPE named node
	struct IntListNode *node = newIntListNode(v);
	//CHECK FOR INPUTTED LIST EMPTY CASE, PUT V IN LIST
	if (l->size == 0) {
		l->first = node;
		l->last = node;
	//IF V IS SMALLEST IN LIST, PUT V IN FIRST PLACE
	} else if (v <= l->first->data) {
		node->next = l->first;
		l->first = node;
	//IF V IS BIGGEST IN LIST, PUT V IN LAST PLACE
	} else if (v >= l->last->data) {
		l->last->next = node;
		l->last = node;
	//V IN MIDDLE OF LIST
	} else {
		struct IntListNode *middle;
		middle = l->first;
		//LOOP TO CHECK WHERE V SHOULD BE AT
		while (middle != NULL) {
			if (v <= middle->next->data) {
...
IntList IntListSortedCopy(IntList l) {
	...
	//CREATE A FIRST NODE TYPE intlistnode
	struct IntListNode *first = l->first;

	IntList newlist = IntListNew();
	while (first != NULL) {
		IntListInsertInOrder(newlist, first->data);
		first = first->next;
	}
