...
extend qs = [qs ++ [q] | q <- [0..7],safe q qs]
  where
    safe :: Row -> Queens -> Bool
    safe row qs = not (elem row qs || sameDiagonal row qs)
    
    sameDiagonal :: Row -> Queens -> Bool
    sameDiagonal row qs = any (\(col, qRow) -> abs (row - qRow) == col) (zip [length qs,length qs - 1..] qs)
...
solutionStartWith p = extendRemaining [p] 1
  where
    extendRemaining :: Queens -> Int -> [Queens]
    extendRemaining qs col
          | col >= 8 = [qs] 
          | otherwise = do
              extended <- extend qs
              extendRemaining extended (col + 1)
