...
{- Feel free to import more stuff here, as long as it's all from
 ...
 -}
import Data.Maybe(fromMaybe, isNothing, isJust, fromJust, catMaybes, mapMaybe)
...
wellFormed (Trie _ xs) = 
    all (wellFormed . snd) xs && 
    sort chars == chars &&
    nub chars == chars
  where 
    chars = map fst xs
...
{- We say that a trie is *minimal* if it contains no
 ...
 -}
minimal :: Trie -> Bool
minimal t = t == empty || minimal' t
  where
    minimal' (Trie b []) = b
    -- check all branch is minimal
    minimal' (Trie _ ts)     = all (minimal' . snd) ts
...
{- Write a `prune t` which returns a minimal
 ...
 -}
prune :: Trie -> Trie
prune (Trie b xs) = Trie b xs'
  where 
    xs' = filter ((/= empty) . snd) $ map (\(c, t) -> (c, prune t)) xs 
...
union (Trie b1 xs) (Trie b2 ys) = Trie (b1 || b2) branches
  where 
    xys = xs ++ ys
    allChars = nub $ sort $ map fst xys
    branches = map (\c -> (c, foldl union empty $ map snd $ filter((==c).fst) xys)) allChars

{- The intersection of two dictionaries t,t' should contain
   all words that occur in *both* t and t'.
 -}
intersection :: Trie -> Trie -> Trie
intersection t1 t2 = Trie (b1 && b2) branches
  where 
    -- Considering that the input may not be wellformed, first union an empty to make the first layer become wellformed
    Trie b1 xs = union t1 empty
    Trie b2 ys = union t2 empty
    -- Find the same branch and merge it recursively
    branches = concatMap (\(c, bs) -> 
        case lookup c ys of
          Nothing -> []
          Just bs' -> [(c, intersection bs bs')]
      ) xs
...
instance Semigroup TrieMonoid where
  TrieMonoid l <> TrieMonoid r = TrieMonoid (union l r)
...
pick xs x = if elem x xs then (True, delete x xs) else (False, xs)
...
sandwichableLetters (Trie _ bs) [] ys = map fst $ filter (\(c, t) -> check t ys) bs
sandwichableLetters (Trie _ bs) (x:xs) ys = nub $ concatMap (\(c, t) -> 
    if c == x then sandwichableLetters t xs ys else []
  ) bs
...
filterLength 0 (Trie b _) = Trie b []
-- At the penultimate layer, only the branch that is true for the next layer is taken
filterLength 1 (Trie b bs) = Trie False $ filter (\(_, Trie b _) -> b) $ map (\(c, t) -> (c, filterLength 0 t)) bs
-- In other cases, filter the empty branch (regardless of the value is true or false), and then set the value to false
filterLength n (Trie _ bs) = Trie False $ filter (\(_, Trie _ x) -> not (null x)) $ map (\(c, t) -> (c, filterLength (n-1) t)) bs
...
filterPattern p = filterPattern' p . filterLength (length p)
  where
    filterPattern' :: Pattern -> Trie -> Trie
    filterPattern' [] t = t
    filterPattern' (Wildcard:cs) (Trie b bs) = Trie b $ map (\(c, t) -> (c, filterPattern' cs t)) bs
    filterPattern' ((Mem xs):cs) (Trie b bs) = Trie b $ map (\(c, t) -> (c, filterPattern' cs t)) $ filter (\(c, t) -> elem c xs) bs
...
filterPlayables [] (Trie b _) = Trie b []
filterPlayables xs (Trie b bs) =Trie b [
    (c, filterPlayables xs' t)| 
    (c, t) <- bs,
    (ok, xs') <- [pick xs (Letter c), pick xs Blank],
    ok
  ]
...
moves t n r (Board b) | n <= 0 || length r < n || length (filter (isNothing.snd) b) < n = empty
  |otherwise = horizontals
    where 
      -- Converts the fixed letters and vertical limits of the main row into a pattern
      createPattern :: Int -> [((String,String),Maybe Char)] -> Pattern
      createPattern 0 xs = map (\x -> Mem [fromJust x]) $ takeWhile isJust $ map snd xs
      createPattern n (((as, bs),Nothing):xs) = (if length (as ++ bs) == 0 then Wildcard else Mem (sandwichableLetters t as bs)) : createPattern (n-1) xs
      createPattern n ((_,Just x):xs) = Mem [x] : createPattern n xs

      -- A trie that satisfies the fixed letters of the main row and the vertical limiting pattern
      pattern' = createPattern n b
      patternChars = mapMaybe snd $ take (length pattern') b
      patternFiltered = filterPattern pattern' t
      -- horizontal results
      horizontals = filterPlayables (map Letter patternChars ++ r) patternFiltered
...
allMoves moves t r b = foldr union empty $ map (\n -> moves t n r b) [1..length r]
