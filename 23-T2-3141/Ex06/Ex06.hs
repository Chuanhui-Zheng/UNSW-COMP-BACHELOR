...
onlyUnique inputFile outputFile = do
  ...
  let uniqueLines = getUniqueLines inputLines
  writeFile outputFile (unlines uniqueLines)

getUniqueLines :: [String] -> [String]
getUniqueLines [] = [] 
getUniqueLines (x:xs)
  | x `elem` xs = getUniqueLines (filter (/= x) xs)
  | otherwise = x : getUniqueLines xs

-- Example usage:
-- onlyUnique "input.txt" "output.txt"
...
ai = Player { guess = aiGuess, signalWrong = aiWrong } where
  ...
  aiWrong response = do
    AiState lower upper <- get
    case response of
      Lower -> put (AiState lower ((lower + upper) `div` 2 -1))
      Higher -> put (AiState ((lower + upper) `div` 2 +1) upper)
