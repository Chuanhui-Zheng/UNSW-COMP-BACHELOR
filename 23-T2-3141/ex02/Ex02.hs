...
testUnit x = and [mempty <> x == x, x <> mempty == x]
...
minMonoidOp (EmbMM x) (EmbMM y) = EmbMM (x `min` y)
...
isSparseList ((n, x) : (m, y) : xs)  = n > 0 && x /= y && isSparseList ((m, y) : xs)
...
append (SparseList ((n, x) : xs)) (SparseList ((m, y) : ys))
  | null xs                = if x == y then SparseList $ (n + m, x) : ys else lst
  | otherwise              = lst
  where
    lst = let SparseList result = append (SparseList xs) (SparseList ((m, y) : ys)) in SparseList $ (n, x) : result
...
testSparseList xs ys = let SparseList zs = append xs ys in isSparseList zs
...
testUnitSparseList x = and [x <> mempty == x, mempty <> x == x]
