...
{- `first ps` behaves as the first parser
 ...
 -}
-- first [a,b,c] = a `orelse` b `orelse` c `orelse` abort
...
whiteSpace :: Parser ()
whiteSpace = void $ parsePred isSpace 
...
parseBool = (True <$ keyword "true") `orelse` (False <$ keyword "false")
...
-- read after parsePred
parsePositiveInt :: Parser Int
parsePositiveInt = do
  xs <- parsePred isDigit
  if null xs || all (=='0') xs then abort else return $ read xs
...
parseDouble = do
  -- parse optional sign
  sign <- ("-" <$ keyword "-") `orelse` return ""
  xs <- parsePred isDigit
  if null xs then abort else return ()
  -- parse optional decimal point and following numbers
  dec <- (do
    keyword "."
    digits <- parsePred isDigit
    if null digits then abort else return()
    return $ "." ++ digits) `orelse` return ""
  return $ read (sign ++ xs ++ dec)
...
parseString = do
  ...
  let notQuote = peekChar >>= (\x -> if x `elem` "\\\"" then abort else return () ) >> parsePred (`notElem` "\\\"")
  -- read escape character
  let quote = keyword "\\"  >> parseChar >>= \c -> return ['\\',c]
  chars <- parseWhile (first [notQuote, quote])
  keyword "\""
  -- Convert quoted string to string, use read directly
  return $ read $ "\""++concat chars++"\""
...
parseList l r elememt = do
  keyword [l]
  whiteSpace 
  let values isfirst = 
        ([] <$ keyword[r]) `orelse` 
        (do 
          if isfirst then return () else keyword ","
          whiteSpace 
          v <- elememt
          whiteSpace
          vs <- values False
...
toSubmission (JSON d) = do
  session <- lookup "session" d >>= getString
  quizName <- lookup "quiz_name" d >>= getString
  student <- lookup "student" d >>= getString
  answers <- lookup "answers" d >>= getList >>= mapM (\x -> getList x >>= mapM getInt)
  time <- lookup "time" d >>= getString >>= toTime
  return $ Submission session quizName student answers time
  where 
    getInt = fmap round . getNumber
...
toSubmissions (JSON xs) = mapM (\(k,v) -> 
  case v of
    JSONData j -> do
      s <- toSubmission j
      return (k,s)
...
parseQuiz = do
  -- parse time
  time <- readLine >>= try . toTime
  questions <- parseWhile questionParser
  -- Check whether the question numbers are consecutive and start from 1
  if not (and $ zipWith (\x y -> number x == y) questions [1..])
    then abort
    else return ()
  -- Check if all answers are positive integers and there is at least one answer
  if not (and $ map (\x -> all (>0) (correct x) && not (null (correct x))) questions)
    then abort
    else return ()
  ...
  where 
    ...
    questionParser = do
      ...
      keyword "|"
      -- parse question type
      t <- first [Radio <$ keyword "radio", CheckBox <$ keyword "checkbox"]
      keyword "|"
      -- parse first answer
      c1 <- parsePositiveInt
      -- parse following answers
      cs <- parseWhile (whiteSpace >> keyword "," >> whiteSpace >> parsePositiveInt)
      keyword "\n" `orelse` return ()
      return $ Question n t (c1:cs)
...
markQuestion (Question _ t correct) ans
  | t== Radio = if correct == ans then 1 else 0
  | otherwise = max 0 ((fromIntegral right - fromIntegral wrong) / fromIntegral (length correct))
    where 
      right = length $ filter (`elem` correct) ans
      wrong = length $ filter (`notElem` correct) ans
...
markSubmission (Quiz dl qs) (Submission _ _ _ answers time ) 
  | time > dl = 0
  | otherwise = sum $ zipWith markQuestion qs answers
...
-- parse the title and submission, then calculate the score, and format the output
...
marker quizStr submissionsStr = do
  ...
  return $ unlines $ map (make1 quiz) submissions
  where
    -- calculate scores for each student
    make1 quiz (id,Submission session quizName student answers time) =
       id ++ "|" ++ quizName ++ "|" ++ show (markSubmission quiz (Submission  session quizName student answers time))
