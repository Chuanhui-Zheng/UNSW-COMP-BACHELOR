...
class MenuScene extends Scene {
  ...
  create() {
          ...
          preConfirm: async () => {
            let inputValidator = async (values) => {
              if (!values || !values[0] || !values[1] || !values[2] || !values[3]) {
                Swal.showValidationMessage("enter digit");
                return false;
              }
              if (!values || !values[4]) {
            ...
            if (await inputValidator(results)) {
              dungeon = await API.generateDungeon(parseInt(results[0]), parseInt(results[1]), parseInt(results[2]), parseInt(results[3]), results[4]);
              return !!dungeon;
            }
          },
        });
    ...
    this.tweens.add({
      delay: 3500,
      duration: 1000,
      targets: [this.newGame, this.credits, this.exitGame, this.generateDungeon],
      x: 10,
      ease: "Bounce",
    });
