...
static void doTreeSearchBetween(Tree t, Node n, Record lower,
    ...
    int cmp_l = t->compare(n->rec, lower);
    int cmp_u = t->compare(n->rec, upper);
    // when is greater than lower
    if (cmp_l > 0) {
        doTreeSearchBetween(t, n->left, lower, upper, l);
    }
    // when is right node we are looing for
    // add to the list
    if (cmp_l >= 0 && cmp_u <= 0) {
        ListAppend(l, n->rec);
    }
    // when is smaller than upper
    if (cmp_u < 0) {
