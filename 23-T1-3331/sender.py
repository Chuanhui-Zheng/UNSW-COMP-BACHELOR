from const import *
import socket, time, threading, sys, copy, os
from utility import *
...
# sender_port, receiver_port, FileToSend, max_win, rto = 54321,56789,  "random2.txt", 4000, 2000  
rto /= 1000
...
def rece(): # ACK
    ...
    try:
        while True:
            ...
            if seq_num_int not in ack_collection:
                ack_collection.append(seq_num_int)
            else:
                summary_dict['ACK_Seg_Received_Repeat'] += 1
            try:
            ...
            if type_int == ACK_NUM:
                for i in range(left , righ):
                    if (window_data[i]['send_index'] + window_data[i]['data_len']) % 2**16 == ((seq_num_int) % 2**16 ) :  
...
try:
    ...
    rece_ack = False
    while not rece_ack:  
        if (seq_num_int) % 2**16 == (ISN + 1) % 2**16:
            s = SEQ(ISN + 1)
            s.seq %= 2**16
            rece_ack = True
            break
    ...
    while rece_ack and left < N:
        ...
        while righ < N and righ - left < int(window_size) : # mv rig
            ...
            s.seq %= 2**16
            righ += 1
        if  time.time() - window_data[left]['time_stamp'] > rto: # ot
    ...
    if not reset:
        ...
        while rece_ack and not rece_fin:  
            if (seq_num_int) % 2**16 == (se + 1) % 2**16:
                rece_fin = True
                break
            else:
                for i in range(3):
                    ...
                    if (seq_num_int) % 2**16 == (se + 1) % 2**16:
                        ...
                        send('FIN', se)
                break
        if rece_ack and not rece_fin:
            send('RESET', se)
except socket.error:
    pass

summary_dict['Amout_Data_No_Repeat'] = len(send_all_message) if rece_ack else 0
summary_dict['Data_Seg_Sent_No_Repeat'] = N if not reset else 0
