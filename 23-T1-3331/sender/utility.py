...
def write_log(snd_or_rcv, timey, type_num, seq_num, data_message=''):
    ...
    return f'{snd_or_rcv:<4} {timey:<8.6f} \t{lst[type_num]} {seq_num:>5} {length}\n'

def increase(orign, interval = 1):
    return (orign + interval) % 65536

def Abs_SeqToRela_Seq(abs_seq):
    diff = abs_seq - ISN -1
    if diff // MSS == diff / MSS:
        return diff // MSS
    else:
        return diff // MSS + 1
...
def absToRela(abs):
    if abs < 1000:
        ...
        diff = abs - ISN + 1
        rela = diff // 1000 if diff // 1000 == diff / 1000 else diff // 1000 + 1
        return rela 